...
let
  ...
  recycle_prefix = "transform-prompt[sh -c '\\''prompt=\"$FZF_PROMPT\"; case \"$prompt\" in \"♻️\"*) base=\"\${prompt#♻️}\" ;; *) base=\"$prompt\" ;; esac; if [ -f ${sesh_fzf_recycle_flag} ]; then printf \"♻️%s\" \"$base\"; else printf \"%s\" \"$base\"; fi'\\'']";
...
in
{
  ...
  xdg.configFile."sesh/sesh.toml".text = # toml
    ''
      ...
      [[session]]
      name = "configuration"
      startup_command = "tmux rename-window system && tmux set-option status on && git pull"
